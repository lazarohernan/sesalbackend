...
exports.ejecutarConsultaPivot = exports.obtenerValoresDimension = exports.obtenerAniosDisponibles = exports.obtenerCatalogoPivot = void 0;
const pool_1 = require("../base_datos/pool");
const tomarPool = () => (0, pool_1.obtenerPoolActual)();
const DETALLE_PREFIX = "AT2_BDT_MENSUAL_DETALLE_";
const JOIN_DEFINITIONS = {
    us: "LEFT JOIN BAS_BDR_US us ON us.C_US = det.C_US",
    cat_concepto: "LEFT JOIN cat_conceptos cat_concepto ON cat_concepto.codigo COLLATE utf8mb4_unicode_ci = det.C_CONCEPTO COLLATE utf8mb4_unicode_ci",
    cat_concepto_ordenado: "LEFT JOIN cat_concepto_ordenado cat_concepto_ordenado ON cat_concepto_ordenado.codigo COLLATE utf8mb4_unicode_ci = det.C_CONCEPTO COLLATE utf8mb4_unicode_ci",
    cat_establecimiento: "LEFT JOIN cat_establecimientos cat_establecimiento ON cat_establecimiento.codigo COLLATE utf8mb4_unicode_ci = CAST(det.C_US AS CHAR) COLLATE utf8mb4_unicode_ci",
    cat_region: "LEFT JOIN cat_regiones cat_region ON cat_region.codigo COLLATE utf8mb4_unicode_ci = CAST(us.C_REGION AS CHAR) COLLATE utf8mb4_unicode_ci",
    deptos: "LEFT JOIN BAS_BDR_DEPARTAMENTOS deptos ON deptos.C_DEPARTAMENTO = us.C_DEPARTAMENTO",
    municipios: "LEFT JOIN BAS_BDR_MUNICIPIOS municipios ON municipios.C_DEPARTAMENTO = us.C_DEPARTAMENTO AND municipios.C_MUNICIPIO = us.C_MUNICIPIO",
    cat_nivel_establecimiento: "LEFT JOIN cat_nivel_establecimiento cat_nivel_establecimiento ON cat_nivel_establecimiento.codigo COLLATE utf8mb4_unicode_ci = CAST(us.C_NIVEL_US AS CHAR) COLLATE utf8mb4_unicode_ci",
    cat_nivel_operativo: "LEFT JOIN cat_nivel_operativo cat_nivel_operativo ON cat_nivel_operativo.codigo COLLATE utf8mb4_unicode_ci = CAST(us.C_NIVEL_US AS CHAR) COLLATE utf8mb4_unicode_ci",
    cat_formularios: "LEFT JOIN cat_formularios cat_formularios ON cat_formularios.codigo COLLATE utf8mb4_unicode_ci = det.V_FORMULARIO COLLATE utf8mb4_unicode_ci",
    concepto_ge: "LEFT JOIN AT2_BDR_CONCEPTOS_GE concepto_ge ON concepto_ge.C_CONCEPTO = det.C_CONCEPTO AND concepto_ge.V_FORMULARIO = det.V_FORMULARIO"
};
const DIMENSIONES = {
    // ANIO: {
    //   id: "ANIO",
...
const TOTAL_EXPRESSION = "COALESCE(det.Q_AT_ENFERMERA_AUX, 0) + COALESCE(det.Q_AT_ENFERMERA_PRO, 0) + COALESCE(det.Q_AT_MEDICO_GEN, 0) + COALESCE(det.Q_AT_MEDICO_ESP, 0)";
const MEDIDAS = {
    TOTAL: {
        id: "TOTAL",
...
const obtenerTablasDetalleDisponibles = async () => {
    const pool = tomarPool();
    const [rows] = await pool.query(`SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME LIKE '${DETALLE_PREFIX}%'`);
    return rows
        .map((row) => {
...
const asegurarJoins = (joinsNecesarios) => {
    return Array.from(joinsNecesarios)
        .map((clave) => JOIN_DEFINITIONS[clave])
        .filter(Boolean)
        .join("\n");
};
const obtenerPeriodosDisponibles = async () => {
    const pool = tomarPool();
...
const obtenerCatalogoPivot = async () => {
    const pool = tomarPool();
    ...
    for (const dimension of Object.values(DIMENSIONES)) {
        ...
        if (dimension.id === "MES") {
            ...
            const [rows] = await pool.query(`SELECT ${dimension.catalog.valueColumn} AS valor, ${dimension.catalog.labelColumn} AS etiqueta FROM ${dimension.catalog.table} ORDER BY ${dimension.catalog.orderBy ?? dimension.catalog.labelColumn} LIMIT ?`, [limit]);
            base.valores = rows.map((row) => ({ valor: row.valor, etiqueta: row.etiqueta }));
            base.totalValores = rows.length;
        }
        else if (dimension.catalog) {
            const [rows] = await pool.query(`SELECT COUNT(*) AS total FROM ${dimension.catalog.table}`);
    ...
    const medidas = Object.values(MEDIDAS).map((medida) => ({
        id: medida.id,
...
exports.obtenerCatalogoPivot = obtenerCatalogoPivot;
...
exports.obtenerAniosDisponibles = obtenerAniosDisponibles;
const obtenerValoresDimension = async (dimensionId, busqueda, limite = 200) => {
    const pool = tomarPool();
...
exports.obtenerValoresDimension = obtenerValoresDimension;
const normalizarValoresFiltro = (dimension, valores) => {
...
const construirSelectDimensiones = (ids) => {
    ...
    return { selects, groupBy, joins: new Set(joins), orderBy, definiciones };
};
const agregarJoinsDeFiltros = (filtros) => {
    const joins = new Set();
...
const aplicarFiltros = (filtros, dimensionesSeleccionadas, condiciones, parametros) => {
    if (!filtros?.length)
        return;
...
const obtenerYearsDesdeFiltros = (filtros) => {
    if (!filtros)
        return [];
...
const transformarDatosPivot = (datos, dimensionesFilas, dimensionesColumnas, medidas) => {
    if (!datos.length) {
        return {
            cabeceras: [],
            filas: [],
    ...
    if (valoresColumnas.size === 0) {
        ...
        const filas = datos.map(fila => cabecerasData.map(cabecera => fila[cabecera] ?? null));
        return {
    ...
    const filas = Array.from(mapaPivot.values()).map(fila => cabeceras.map(cabecera => fila[cabecera] ?? null));
...
const ejecutarConsultaPivot = async (payload) => {
    const pool = tomarPool();
    const valoresSolicitud = payload.values?.length ? payload.values : [];
    ...
    const condiciones = [];
    const parametros = [];
    const setDimensiones = new Set(dimensionesSolicitadas);
    ...
    if (payload.year !== undefined) {
        ...
        aniosConsulta = Array.from(new Set(aniosFiltro.filter((anio) => aniosDisponibles.includes(anio)))).sort((a, b) => a - b);
    ...
    if (payload.includeTotals && medidas.length) {
        ...
        const [totalRows] = await pool.query(totalSql, parametros);
    ...
    let datosTransformados = rows.map((row) => ({ ...row }));
...
exports.ejecutarConsultaPivot = ejecutarConsultaPivot;
//# sourceMappingURL=pivot.servicio.js.map