...
exports.obtenerDatosMapaHonduras = exports.obtenerResumenTablero = void 0;
const configuracion_1 = require("../configuracion");
const pool_1 = require("../base_datos/pool");
const obtenerResumenTablero = async (anio) => {
    const pool = (0, pool_1.obtenerPoolActual)();
    const [catalogos] = await pool.query(`SELECT
    ...
    if (anio) {
        ...
        try {
            const [detalle] = await pool.query(`SELECT COUNT(*) AS total FROM ${tablaDetalle} WHERE N_ANIO = ?`, [anio]);
        ...
        const [tablas] = await pool.query(`SELECT
         SUM(TABLE_ROWS) AS total
       FROM information_schema.TABLES
       WHERE TABLE_SCHEMA = ?
         AND TABLE_NAME LIKE 'AT2_BDT_MENSUAL_DETALLE_%'`, [configuracion_1.entorno.baseDatos.nombre]);
...
exports.obtenerResumenTablero = obtenerResumenTablero;
const obtenerDatosMapaHonduras = async () => {
    const pool = (0, pool_1.obtenerPoolActual)();
...
exports.obtenerDatosMapaHonduras = obtenerDatosMapaHonduras;
//# sourceMappingURL=tablero.servicio.js.map